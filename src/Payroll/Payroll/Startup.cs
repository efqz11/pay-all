                    ...
                    .CreateScope())
            {
                ...
                if (!accountDbContext.AppUsers.Any())
                {
                    ...
                    if (!roleCheck)
                    {
                        ...
                        RoleManager.CreateAsync(new AppRole(Roles.PayAll.admin)).Wait();
                    }

                    var adminUsserId = "7fadeeb8-f941-11e9-aad5-362b9e155667";
                ...
                additions.ForEach(d => {
                    ...
                    if (!d.IsFilledByEmployee.HasValue) d.VariationType = VariationType.VariableAddition;
                    d.Fields.ForEach(f => f.CalculationIdentifier = f.DisplayName.GenerateSlug());
                });
                dedcutions.ForEach(d => {
                    d.CompanyId = company.Id;
                    d.VariationType = VariationType.VariableDeduction;
                    d.Fields.ForEach(f => f.CalculationIdentifier = f.DisplayName.GenerateSlug());
                });
                ...
                var emp_shan = new Employee { EmpID = "3", FirstName = "Shan", LastName = "Ahmed", JobTitle = "Office Assistant", DateOfJoined = new DateTime(2017, 09, 04), JobType = JobType.FullTime, Department = company.Departments.Last(), DepartmentId = context.Departments.OrderByDescending(q=> q.Id).First().Id };
